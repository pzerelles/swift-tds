extension TDSTokenParser {
  public static func parseColMetadataToken(from buffer: inout ByteBuffer) throws
    -> TDSTokens.ColMetadataToken
  {
    guard
      let count = buffer.readUShort()
    else {
      throw TDSError.protocolError(
        "Invalid COLMETADATA token: Error while reading COUNT of columns")
    }
    ...
    for _ in 0...count - 1 {
      ...
      if dataType.isScaleType() {
        ...
        if let p = precision {
          guard s <= p else {
            throw TDSError.protocolError(
              "Invalid SCALE value. Must be less than or equal to precision value.")
          }
        }
      ...
      colData.append(
        TDSTokens.ColMetadataToken.ColumnData(
          userType: userType, flags: flags, dataType: dataType, length: length,
          collation: collationData, tableName: tableName, colName: colName, precision: precision,
          scale: scale))
    }
