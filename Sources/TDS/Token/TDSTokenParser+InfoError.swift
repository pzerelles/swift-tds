extension TDSTokenParser {
  public static func parseErrorInfoToken(type: TDSTokens.TokenType, from buffer: inout ByteBuffer)
    throws -> TDSTokens.ErrorInfoToken
  {
    guard
    ...
    let token = TDSTokens.ErrorInfoToken(
      type: type, number: Int(number), state: state, classValue: classValue, messageText: msgText,
      serverName: serverName, procedureName: procName, lineNumber: Int(lineNumber))
