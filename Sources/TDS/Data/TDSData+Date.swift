  ...
  public var date: Date? {
    ...
    case .datetime:
      guard
        ...
        let oneThreeHundrethsOfASecondElapsed = value.readInteger(
          endianness: .little, as: UInt32.self)
      else {
        return nil
      }
    ...
    case .datetimeOffset:
      ...
      guard
        ...
        let timezoneOffset = value.readInteger(as: Int16.self),
...
extension ByteBuffer {
  ...
  fileprivate mutating func readTimeComponents(bytes length: Int, scale: Int?) -> DateComponents? {
    guard var secondIncrements: Int = self.readByteLengthInteger(length: length), let scale = scale
    else {
      return nil
    }
