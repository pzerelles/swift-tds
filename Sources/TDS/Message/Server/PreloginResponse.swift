  ...
  public struct PreloginResponse: TDSMessagePayload {
    ...
    public static func parse(from buffer: inout ByteBuffer) throws -> PreloginResponse {
      ...
      while readOptions {
        ...
        guard let mappedToken = _buffer.readByte().map(PreloginToken.init), let token = mappedToken
        else {
        ...
        guard
          ...
          throw TDSError.protocolError(
            "Invalid Prelogin Response: Invalid *PRELOGIN_OPTION segment.")
        }
      ...
      for option in preloginOptions {
        guard
          ...
          throw TDSError.protocolError(
            "Invalid Prelogin Response: Error while parsing PL_OPTION_DATA")
        }
        ...
        case .encryption:
          ...
          guard
            ...
            throw TDSError.protocolError(
              "Invalid Prelogin Response: Invalid ENCRYPTION option data.")
          }
      ...
      guard let lastOption = preloginOptions.last else {
        throw TDSError.protocolError(
          "Invalid Prelogin Response: Should be at least 1 PRELOGIN_OPTION.")
      }
