...
extension TDSMessage {
  init(
    from buffer: inout ByteBuffer, ofType type: TDSPacket.HeaderType, allocator: ByteBufferAllocator
  ) throws {
    ...
    while buffer.readableBytes >= TDSPacket.maximumPacketDataLength {
      ...
      packets.append(
        TDSPacket(
          from: &packetData, ofType: type, isLastPacket: false, packetId: packetId,
          allocator: allocator))
      packetId = packetId &+ 1
    }
    ...
    packets.append(
      TDSPacket(
        from: &lastPacket, ofType: type, isLastPacket: true, packetId: packetId,
        allocator: allocator))
