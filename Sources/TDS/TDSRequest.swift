...
final class TDSRequestHandler: ChannelDuplexHandler {
  ...
  private func _channelRead(context: ChannelHandlerContext, data: NIOAny) throws {
    ...
    do {
      let response = try request.delegate.handle(
        packet: packet, allocator: context.channel.allocator)
      switch response {
      case .kickoffSSL:
        guard case .sentPrelogin = state else {
          throw TDSError.protocolError(
            "Unexpected state to initiate SSL kickoff. If encryption is negotiated, the SSL exchange should immediately follow the PRELOGIN phase."
          )
        }
  ...
  private func sslKickoff(context: ChannelHandlerContext) throws {
    ...
    let coordinator = PipelineOrganizationHandler(
      logger: logger, firstDecoder, firstEncoder, sslHandler)
    ...
    context.channel.pipeline.addHandler(coordinator, position: .before(self)).whenComplete { _ in
      context.channel.pipeline.addHandler(sslHandler, position: .after(coordinator)).whenComplete {
        _ in
  ...
  private func write(
    context: ChannelHandlerContext, packets: [TDSPacket], promise: EventLoopPromise<Void>?
  ) throws {
    var packets = packets
    ...
    case .prelogin:
      ...
      case .sentPrelogin, .sslHandshakeStarted, .sslHandshakeComplete, .sentLogin, .loggedIn:
        throw TDSError.protocolError(
          "PRELOGIN message must be the first message sent and may only be sent once per connection."
        )
      }
    case .tds7Login:
      switch state {
      ...
      case .start, .sslHandshakeStarted, .sentLogin, .loggedIn:
        throw TDSError.protocolError(
          "LOGIN message must follow immediately after the PRELOGIN message or (if encryption is enabled) SSL negotiation and may only be sent once per connection."
        )
      }
    default:
      break
  ...
  private func _userInboundEventTriggered(context: ChannelHandlerContext, event: Any) throws {
    if let sslHandler = sslClientHandler, let sslHandshakeComplete = event as? TLSUserEvent,
      case .handshakeCompleted = sslHandshakeComplete
    {
      ...
      let future = EventLoopFuture.andAllSucceed(
        [
          ...
          context.channel.pipeline.addHandler(
            ByteToMessageHandler(TDSPacketDecoder(logger: logger)), position: .after(sslHandler)),
          context.channel.pipeline.addHandler(
            MessageToByteHandler(TDSPacketEncoder(logger: logger)), position: .after(sslHandler)),
