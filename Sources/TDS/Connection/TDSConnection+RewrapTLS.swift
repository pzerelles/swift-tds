...
public final class PipelineOrganizationHandler: ChannelDuplexHandler, RemovableChannelHandler {
  ...
  private func _write(
    ...
    case .start:
      let sslHandshakeState = SSLHandshakeState(
        ...
        outputPromise: context.eventLoop.makePromise())
  ...
  private func _flush(context: ChannelHandlerContext) throws {
    switch self.state {
    case .sslHandshake(var sslHandshakeState):
      let message = try TDSMessage(
        from: &sslHandshakeState.outputBuffer, ofType: .prelogin,
  ...
  private func updateSSLHandshakeState(
    _ sslHandshakeState: SSLHandshakeState, data: NIOAny, promise: EventLoopPromise<Void>?
  ) {
